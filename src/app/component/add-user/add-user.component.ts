    ...
    if(this.activatedRoute.snapshot.params['id'] != undefined) {
      this.dataService.currentPerson.subscribe(person => {
        if(Object.keys(person).length !== 0) {
          console.log(person);
          this.personFormGroup.get('fullName');
          this.personFormGroup.get('address');
          this.personFormGroup.get('city');
          this.personFormGroup.get('state');
          this.personFormGroup.get('zipNo');
          this.personFormGroup.get('phoneNumber');
        }
      });
    }
  }
  ...
  this.person = this.personFormGroup.value;
    if(this.activatedRoute.snapshot.params['id'] != undefined){
      ...
      this.httpService.addNewContact(this.person).subscribe(response => {
        console.log(response);
        this.router.navigateByUrl("/home-page");
        this.notifierService.showNotification('Added One Person Successfully!', '', 'success');
      });
    }
  }
}
